...
/*bool MyoController::updatePitch(){
	if (Serial.available())
	{
		storagePitchVal = String("");
		while(Serial.available())
		{
			storagePitchVal = storagePitchVal + char(Serial.read());
			delay(1);
		}
		
		msgChar = storagePitchVal;
	}
		current_pitch_ = atoi(msgChar.c_str());
}*/
...
int MyoController::getCurrentPitch(){
